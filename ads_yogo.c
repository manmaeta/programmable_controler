...
int  main()
{
    ...
    time_t start,end;
    double total =0.0, setTime=180.0; 
    ...
    char filepath[256];
    sprintf(filepath,"data/controller:Y%.2d_M%.2d_D%.2d_T%.2d:%.2d.txt", systime->tm_year + 1900, systime->tm_mon+1,
             systime->tm_mday, systime->tm_hour , systime->tm_min);

    outputfile = fopen(filepath,"w");

    fprintf(outputfile,"DATE: Y%.2d_M%.2d_D%.2d_T%.2d:%.2d.txt", systime->tm_year + 1900, systime->tm_mon+1,
             systime->tm_mday, systime->tm_hour , systime->tm_min);
             
    fprintf(outputfile,"Time,            Volts(ch7)[V],\n"); 
...
*/
   ...
   printf("ID=\r\n");  
		...
		while(1)
	{
		...
		t = time(NULL);    //time_mnejar
        systime = localtime(&t); 
        
        if(systime->tm_sec != beforetime){
			
			beforetime=systime->tm_sec;
			
			fprintf(outputfile,"%.2d:%.2d:%.2d,          ", systime->tm_hour,  // Write Date to CSV file 
                 systime->tm_min, systime->tm_sec);
        
			iTemp = volt[i-1]; //pin nunber 7   
			                     
			fprintf(outputfile,"%ld.%03ld%03ld\r\n", iTemp /1000000, (iTemp%1000000)/1000, iTemp%1000); // Write output voltage to CSV file
		}
		end= time(NULL);
		...
		if(total > setTime){    //finish mesurement
			fclose(outputfile);
			break;
		}	
	}	
